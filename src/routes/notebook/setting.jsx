...
export default function SettingPanel({ id, close, onClose, onDone }) {
	...
	return (
		<PopupPanel
			...
			onDone={() => {
				updateBook(id, { name: data.name || "", desc: data.desc || "" });
				onDone();
			}}
		>
			<div
				className="middle"
				style={{ padding: "10px", transform: "scale(0.9)" }}
			>
			...
			<div className="nb-setting-body">
				...
				<Editable
					...
					label="Name"
					disabled={true}
					onEdit={(value) => setData({ ...data, name: value || "" })}
				/>
				<Editable
					value={data.desc}
					label="Description"
					disabled={true}
					onEdit={(value) => setData({ ...data, desc: value || "" })}
				/>
			...
			>
				<div className="p">
					Delete the NoteBook <b>{data.name}</b>
				</div>
			</PopupPanel>
		</PopupPanel>
	);
