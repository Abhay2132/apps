...
export default function Editor({ bookID, onBack, update }) {
	...
	const setMD = (content, f) => {
		...
		md.current.innerHTML = marked.parse(
			content.replace(/^[\u200B\u200C\u200D\u200E\u200F\uFEFF]/, "")
		);
	};
	...
	return (
		<div id="editor" mode={data.md ? "md" : "edit"}>
			...
			<textarea
				...
				onInput={() => {
					setData({ ...data, text: ref.current.value });
					edited.current = true;
				}}
				value={data.text}
			></textarea>
			<div id="editor-md" className="markdown-body" ref={md}>
				MARKDOWN
			</div>
