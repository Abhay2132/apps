...
export default function Component(argument) {
	...
	function startDL() {
		...
		window.open(
			`${host}/ytdl/dl?url=${url}&q=${
				data.type == "video" ? response.vqs[data.video].height : data.audio
			}&a=${data.type == "audio" ? "1" : ""}`
		);
	}
	...
	const parseSize = (s) =>
		s
			? "~ " + (parseInt(s) / (1024 * 1024)).toFixed(1) + " MB"
	...
	return (
		<div id="ytdl-box" className="app" data-state={formState}>
			...
			<ErrorPanel>
				<center>Try Resubmit the Link</center>
			</ErrorPanel>
			<LoadingPanel />
			<div className="data-panel" id="ytdl-data">
				...
				<div
					id="videoThumbnail"
					style={{
						background: Boolean(response.thumbnail)
							? `url("${response.thumbnail}")`
							: "#000",
					}}
				></div>
				<div id="ytdl-data-grid">
					<div className="ytdl-data-col1">Name</div>
					<div className="ytdl-data-col2 center-V">{response.title}</div>
					<div className="ytdl-data-col1">Duration</div>
					<div className="ytdl-data-col2 center-V">
						{parseTime(response.dur)}
					</div>
					<div className="ytdl-data-col1">TYPE</div>
					<div className="ytdl-data-col2">
						<Option
							state={data.type == "video" ? "active" : ""}
							onClick={() => setData({ ...data, type: "video" })}
						>
							VIDEO
						</Option>
						<Option
							state={data.type == "audio" ? "active" : ""}
					...
					<div className="ytdl-data-col2" data-type={data.type}>
						<div className="video-qualities">
							{videos.map((vq, i) => (
								<Option
									state={data.video == vq ? "active" : ""}
									onClick={() => setData({ ...data, video: vq })}
									key={i}
									text={vq}
								/>
							))}
						...
						<div className="audio-qualities">
							{audios.map((aq, i) => (
								<Option
									state={data.audio == aq ? "active" : ""}
									key={i}
									text={aq}
									onClick={() => setData({ ...data, audio: aq })}
								/>
							))}
						</div>
					</div>
