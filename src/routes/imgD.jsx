...
export default function () {
	...
	function postData(data) {
		if (!data || !Array.isArray(data) || data.length < 1)
			return new Error("invalid 'urls' data !");
	...
	return (
		<div className="app imgD-box" imgd={imgD}>
			<form onSubmit={cancleDefault} className="imgD-form" autoComplete="off">
				<h2 onClick={logData} className="imgD-h2">
					{" "}
					Enter URLs{" "}
				</h2>
				<div id="urls-box" ref={urlsBox}>
					...
					<label ref={label} className="urls-box-footer">
						* Paste new url
					</label>
				</div>
				<Button bg="rgba(0,0,0,0.8)" onClick={addURL}>
					Add More
				</Button>
	...
	return (
		<div id="result-panel">
			...
			<div className="ir-info">
				<img src={imgIcon} height="30px" />
				<span> {data.imgs} Images</span>
			</div>
			<div className="ir-info">
				<img src={pageIcon} height="30px" />
				<span> {data.pages} Webpages</span>
			</div>
			<center>
				<button onClick={() => startDL(data.token)}>Download Zip</button>
			</center>
		</div>
	);
}
